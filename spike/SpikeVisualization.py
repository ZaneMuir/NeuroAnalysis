...
def plot_curve_with_error_ribbon(mean_response, roi, ci='std', error_alpha=0.1, markers=None, label=''):
    _mean = np.mean(mean_response, axis=0)
    _ci = _calc_error(mean_response, ci=ci)

    x = np.linspace(roi[0],roi[1],len(_mean))
    plt.plot(x, _mean, label=label)
    plt.fill_between(x, _mean-_ci, _mean+_ci, alpha=error_alpha)
    if markers:
        markers()
    plt.xlim(roi)
    return
